package com.antbrains.saramtjj.extractor;
...
public class Level0Extractor extends BasicInfoExtractor{
	...
	public List<Block> extractBlock(WebPage webPage, NekoHtmlParser parser, HttpClientFetcher fetcher, String content,
		...
		NodeList aList=parser.selectNodes("//DIV[@class='list_sort']//A");
		...
		for(int i=1;i<=lastPage;i+=maxCrawlThread){
			...
			for(int pg=startPage;pg<endPage;pg++){
				String pageUrl=urlTemp+"/index"+pg+".htm";
	...
	public List<ListPageItem> extractItems(String url, NekoHtmlParser parser, Archiver archiver, String taskId,
			String content) {
		List<ListPageItem> items = new ArrayList<>(34);
		NodeList divList=parser.selectNodes("//DIV[@class='con_list3']//LI");
		for(int i=0;i<divList.getLength();i++){
			...
			int beg=anchorText.indexOf("'");
			int end=anchorText.indexOf("'",beg+1);
			anchorText=anchorText.substring(beg+1, end);
			String pubTime=parser.getNodeText("./SPAN", div).trim();
	...
	public static void main(String[] args) {
		String[] urls = new String[] { "http://www.sara.gov.cn/mtjj/index.htm", };
