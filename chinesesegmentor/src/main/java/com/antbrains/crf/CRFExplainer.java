...
public class CRFExplainer extends JFrame {
	...
	private void draw() {
		...
		try {
			...
			Object v1 = graph.insertVertex(parent, "vStart", "Start", xStart, yStart + totalHeight / 2, nodeWidth,
					nodeHeight, nodeStyle);
			startCell = (mxCell) v1;
			...
			for (int i = 0; i < charNum; i++) {
				...
				for (int j = 0; j < labelNum; j++) {
					...
					Object v2 = graph.insertVertex(parent, "v" + i + "," + j,
							curToken + "/" + label + "(" + scoreStr + ")", xoffset, yoffset, nodeWidth, nodeHeight,
							nodeStyle);
			...
			xoffset += nodeWidth * 2;
			Object v3 = graph.insertVertex(parent, "vEnd", "End", xoffset, yStart + totalHeight / 2, nodeWidth,
					nodeHeight, nodeStyle);
