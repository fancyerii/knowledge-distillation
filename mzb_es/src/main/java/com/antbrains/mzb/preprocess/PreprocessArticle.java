package com.antbrains.mzb.preprocess;
...
import com.antbrains.mzb.data.Article;
import com.antbrains.mzb.tools.IntervalParser;
import com.antbrains.mzb.tools.UrlTools;
import com.antbrains.mzb.tools.gson.GsonTool;
...
public class PreprocessArticle {
	...
	Article parseNews(NekoHtmlParser p, String url){
		...
		String title=p.getNodeText("//DIV[@class='listcon']/SPAN[@id='ReportIDname']");
		if(title.isEmpty()) {
			logger.warn("title is null");
			return null;
			}
		article.setTitle(title);
		String pubStr=p.getNodeText("//DIV[@class='listcon']//SPAN[@id='ReportIDIssueTime']").trim();
		...
		String source=p.getNodeText("//DIV[@class='listcon']//SPAN[@id='ReportIDMediaName']").trim();
		article.setSource(empty2Null(source));
		int comments=p.selectNodes("//DIV[@class='listcon']//SPAN[@id='TalkbackIDTSubject']").getLength();
		
		article.setComments(comments);
		Node contentNode=p.selectSingleNode("//SPAN[@id='CurrentlyText']");
		if(contentNode==null) {
		...
		String author=p.getNodeText("//DIV[@class='listcon']//SPAN[4]/SPAN").trim();
		if(!author.isEmpty()){
			article.setAuthors(new String[]{author});
		}
		NodeList imgs=p.selectNodes("//SPAN[@id='ReportIDtext']//IMG");
		if(imgs.getLength()>0){
