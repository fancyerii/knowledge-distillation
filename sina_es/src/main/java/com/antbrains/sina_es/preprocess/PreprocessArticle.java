package com.antbrains.sina_es.preprocess;
...
import com.antbrains.sina_es.tools.UrlTools;
...
public class PreprocessArticle {
	...
	private Pattern ptn=Pattern.compile("(\\d+)年(\\d+)月(\\d+)日 (\\d+):(\\d+)");// ("(\\d+)年(\\d+)月(\\d+)日 (\\d+):(\\d+)");
	...
	Article parseNews(NekoHtmlParser p, String url){
		...
		String title=p.getNodeText("//H1[@id='artibodyTitle']");
		if(title.isEmpty()) {
		...
		String source=p.getNodeText("//SPAN[@id='art_source']").trim();
		if(source==null||source.isEmpty())
			source=p.getNodeText("//DIV[@class='artInfo']//SPAN[@id='media_name']//SPAN").trim();
		...
		Node contentNode=p.selectSingleNode("//DIV[@id='artibody']");
		if(contentNode==null) {
		...
		NodeList imgs=p.selectNodes("//DIV[@id='artibody']//IMG");
		if(imgs.getLength()>0){
