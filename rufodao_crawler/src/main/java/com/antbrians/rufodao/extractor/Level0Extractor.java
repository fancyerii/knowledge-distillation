...
public class Level0Extractor extends BasicInfoExtractor{
	...
	public List<Block> extractBlock(WebPage webPage, NekoHtmlParser parser, HttpClientFetcher fetcher, String content,
		...
		NodeList aList=parser.selectNodes("//DIV[@class='pageNav'][2]//A");
		Node pageAnchor=aList.item(aList.getLength()-2);
		String s=pageAnchor.getTextContent().trim();
		...
		for(int i=2;i<=lastPage;i+=maxCrawlThread){
			...
			int end2=webPage.getUrl().lastIndexOf("/");
			String urlTemp=webPage.getUrl().substring(0,end2);
			for(int pg=startPage;pg<endPage;pg++){
				String pageUrl=urlTemp+"/list2015116102442_"+pg+".htm";
	...
	public List<ListPageItem> extractItems(String url, NekoHtmlParser parser, Archiver archiver, String taskId,
			String content) {
		List<ListPageItem> items = new ArrayList<>(50);
		NodeList divList=parser.selectNodes("//DIV[@class='leftList']//LI");
		for(int i=0;i<divList.getLength();i++){
			...
			int beg=href.lastIndexOf("/", end-1);
			String pubTime=href.substring(beg+1,end);
			if(pubTime==""||pubTime.length()!=8)
				continue;
			//logger.warn("href: "+href+" beg: "+beg+" end: "+end+" pubTime: "+pubTime);
			String y=pubTime.substring(0,4);
			String m=pubTime.substring(4,6);
			String d=pubTime.substring(6,8);
			pubTime=y+"-"+m+"-"+d;
