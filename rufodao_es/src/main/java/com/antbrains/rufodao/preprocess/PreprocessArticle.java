package com.antbrains.rufodao.preprocess;
...
import com.antbrains.rufodao.data.Article;
import com.antbrains.rufodao.tools.IntervalParser;
import com.antbrains.rufodao.tools.UrlTools;
import com.antbrains.rufodao.tools.gson.GsonTool;
...
public class PreprocessArticle {
	...
	Article parseNews(NekoHtmlParser p, String url){
		...
		String pubStr=p.getNodeText("//SPAN[@class='a_time']").trim();
		...
		String source=p.getNodeText("//SPAN[@class='a_source']").trim();
		article.setSource(empty2Null(source));
		int comments=0;
		
		article.setComments(comments);
		Node contentNode=p.selectSingleNode("//DIV[@id='Cnt-Main-Article-QQ']");
		if(contentNode==null) {
			logger.warn("contentNode is null");
			return null;
			}
		NodeList nl=p.selectNodes("./P[@style='text-indent: 2em;' or @style='TEXT-INDENT: 2em']", contentNode);
		StringBuilder sb=new StringBuilder("");
		...
		String author=p.getNodeText("//SPAN[@class='a_author']").trim();
		if(!author.isEmpty()){
			article.setAuthors(new String[]{author});
		}
		NodeList imgs=p.selectNodes("//P[@align='center']//IMG");
		if(imgs.getLength()>0){
