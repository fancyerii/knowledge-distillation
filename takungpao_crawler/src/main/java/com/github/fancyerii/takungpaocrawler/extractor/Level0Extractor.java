...
public class Level0Extractor extends BasicInfoExtractor{
	...
	public List<Block> extractBlock(WebPage webPage, NekoHtmlParser parser, HttpClientFetcher fetcher, String content,
		...
		NodeList aList=parser.selectNodes("//DIV[@id='displaypagenum']//A");
		
		Node lastPageAnchor=aList.item(aList.getLength()-2);
		String s=lastPageAnchor.getTextContent().trim();
	...
	public List<ListPageItem> extractItems(String url, NekoHtmlParser parser, Archiver archiver, String taskId,
			String content) {
		List<ListPageItem> items = new ArrayList<>(12);
		NodeList divList=parser.selectNodes("//UL[@class='list']/LI/DIV[2]");
		for(int i=0;i<divList.getLength();i++){
			Node div=divList.item(i);
			Node anchor=parser.selectSingleNode("./H1/A", div);
	...
	public static void main(String[] args) {
		String[] urls = new String[] { "http://bodhi.takungpao.com/topnews/", };
