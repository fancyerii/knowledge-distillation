...
public class ViewWebPageHbase {
	protected static Logger logger = Logger.getLogger(ViewWebPageHbase.class);
	private HConnection conn;

	private ViewWebPageHbase() {
		String zk = ConfigReader.getProps().getProperty("zk");
		if (zk == null)
			throw new RuntimeException("zk is null in cfg.properties");
	...
	private WebPage getWebPage(String url, String dbName) {
		...
		try {
			webPage = SCHbaseTool.getWebPage(conn, dbName, url, true);
			if (webPage == null)
				return null;
			SCHbaseTool.loadBlock(conn, dbName, webPage);
			Set<String> parents = SCHbaseTool.getParent(conn, dbName, url);
			webPage.getOtherInfo().put("#%parents%#", parents);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		}
		return webPage;
	...
	private static ViewWebPageHbase instance;
	static {
		instance = new ViewWebPageHbase();
	}

	public static ViewWebPageHbase getInstance() {
		return instance;
	}
}
