package com.antbrains.takungpao_es.index;
...
public class ArticleIndexer {
	...
	public ArticleIndexer(String inDir, long interval, boolean deleteAfterProcess, String clusterName, String hostName)
			throws Exception {
					...
					if (articles.size() >= batchSize) {
						...
						if (rsp.hasFailures()) {
							for (BulkItemResponse itemRsp : rsp.getItems()) {
								logger.info("indexFail: " + itemRsp.getItemId() + "\t"
										+ this.findUrl(articles, itemRsp.getId()) + "\t" + itemRsp.getFailureMessage());
	...
	private BulkResponse index(List<Article> articles) {
		...
		for (Article article : articles) {
			bulkRequest.add(client.prepareIndex(Constants.indexName, Constants.TYPE_ARTICLE, article.getMd5())
					.setSource(gson.toJson(article)));
		}
