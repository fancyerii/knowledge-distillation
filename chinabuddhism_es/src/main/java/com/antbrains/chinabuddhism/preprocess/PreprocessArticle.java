package com.antbrains.chinabuddhism.preprocess;
...
import com.antbrains.chinabuddhism.data.Article;
import com.antbrains.chinabuddhism.tools.IntervalParser;
import com.antbrains.chinabuddhism.tools.UrlTools;
import com.antbrains.chinabuddhism.tools.gson.GsonTool;
...
public class PreprocessArticle {
	...
	private Pattern ptn=Pattern.compile("(\\d+)-(\\d+)-(\\d+)");// ("(\\d+)年(\\d+)月(\\d+)日 (\\d+):(\\d+)");
	...
	Article parseNews(NekoHtmlParser p, String url){
		...
		String title=p.getNodeText("//DIV[@class='title_rd_r']");
		if(title.isEmpty()) {
			logger.warn("title is null");
			return null;
			}
		article.setTitle(title);
		//String pubStr=p.getNodeText("//SPAN[@id='pub_date']").trim();
		int end=url.lastIndexOf("/");
		int beg=url.lastIndexOf("/", end-1);
		String pubStr=url.substring(beg+1,end);
		...
		String str=p.getNodeText("//DIV[@align='right']").trim();
		logger.warn("url: "+url+"\n str: "+str);
		if(!str.isEmpty()&&str!=null)
		{
			int author_beg=str.indexOf("：");
			int author_end=str.indexOf("来");
			int source_index=str.indexOf(":", author_beg+1);
			String author=str.substring(author_beg+1, author_end);
			String source=str.substring(source_index+1);
			article.setSource(empty2Null(source));
			if(!author.isEmpty()){
			article.setAuthors(new String[]{author});
			}
		}
		//String commentStr=p.getNodeText("//A[@class='more J_Comment_Count_Txt']/SPAN[1]").trim();
		int comments=0;//this.parseInt(commentStr, 0);
		article.setComments(comments);
		
		int tag=1;
		Node contentNode=p.selectSingleNode("//DIV[@class='content_zi']");
		if(contentNode==null) {
			logger.warn("contentNode is null");
			return null;
			}
		NodeList nl=p.selectNodes("./P[not(@align) and not(@style)]", contentNode);
		if(nl.getLength()==0)
			nl=p.selectNodes("/DIV",contentNode);
		StringBuilder sb=new StringBuilder("");
		...
		NodeList imgs=p.selectNodes("//DIV[@class='content_zi']//IMG");
		if(imgs.getLength()>0){
