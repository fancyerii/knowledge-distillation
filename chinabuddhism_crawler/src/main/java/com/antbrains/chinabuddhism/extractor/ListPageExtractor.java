...
public class ListPageExtractor extends BasicInfoExtractor {
protected static Logger logger = Logger.getLogger(Level0Extractor.class);
	...
	public List<Block> extractBlock(WebPage webPage, NekoHtmlParser parser, HttpClientFetcher fetcher, String content,
		...
		NodeList aList=parser.selectNodes("//DIV[@class='page_p']//A");
		...
		for(int i=2;i<=lastPage;i+=maxCrawlThread){
			...
			for(int pg=startPage;pg<endPage;pg++){
				String pageUrl=webPage.getUrl()+"index_"+pg+".html";
	...
	public List<ListPageItem> extractItems(String url, NekoHtmlParser parser, Archiver archiver, String taskId,
			String content) {
		List<ListPageItem> items = new ArrayList<>(25);
		NodeList ulList=parser.selectNodes("//DIV[@id='new_center']//UL");
		for(int i=0;i<ulList.getLength();i++){
			Node ul=ulList.item(i);
			Node anchor=parser.selectSingleNode("./LI[1]/A", ul);
			String href=parser.getNodeText("./@href", anchor).trim();
			String itemUrl=UrlUtils.getAbsoluteUrl(url, href);
			String anchorText=parser.getNodeText(".",anchor).trim();
			String pubTime=parser.getNodeText("./LI[2]",ul);
	...
	public static void main(String[] args) {
		String[] urls = new String[] { "http://www.chinabuddhism.com.cn/xw/jj/", };
